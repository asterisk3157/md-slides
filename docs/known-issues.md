...
>
> **進捗 (2026-05-30):** A-1〜A-3 実装＆検証済 (f8d8356, 9040302)。B-1(UD教科書体フォールバック拡充)・B-2(frontmatter案内＝説明UIのみ) もコード対応＆ローカル検証済 (18bd9b3)。残るは **UD教科書体の Windows 実機での最終確認** と pptx 実機目視 (C)。
...
### B-1. デジタル教科書体が Windows で反映されない／各種フォントが出たり出なかったり（バグ）— 🔧 2026-05-30 コード対応 (18bd9b3)・要 Windows 実機確認

> 対応: プルダウン value を表記ゆれに強いフォールバック列挙に（`UD デジタル教科書体 N-R, UDデジタル教科書体 N-R, UD デジタル教科書体 N, UD デジタル教科書 N, BIZ UDPGothic`）。frontmatter 直書きが `font-family` に流れることは Times New Roman で実証済。UD教科書体の実反映は Mac では確認できないため **Windows 実機で要確認**。なお createMeasure の計測フォント一貫化（canvas にスタック全体を渡す件）は未着手＝次の候補。
...
### B-2. frontmatter `font:` のカスタム指定が不安定・未検証（要望／要検証）— ✅ 2026-05-30 説明UI追加＋反映実証 (18bd9b3)

> 対応: 「frontmatter `font:` で各自PCの任意フォントが使える」案内文を設定シートに追加（方針＝説明UIのみ。localStorage 登録UIは見送り）。frontmatter 直書き→`font-family` 反映を Times New Roman で実証。複数指定でのフォールバックも案内済。
